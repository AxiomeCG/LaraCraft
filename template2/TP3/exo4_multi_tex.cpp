...
int main(int argc, char** argv) {
    ...
    GLuint locateEarthTexture = glGetUniformLocation(program.getGLId(), "uEarthTexture");
    GLuint locateCloudTexture = glGetUniformLocation(program.getGLId(), "uCloudTexture");
    ...
    while (!glfwWindowShouldClose(windowManager)) {
        ...
        glUniform1i(locateEarthTexture, 0);
        glBindTexture(GL_TEXTURE_2D, textures_earth[0]);
        glActiveTexture(GL_TEXTURE1);
        glUniform1i(locateCloudTexture, 1);
        glBindTexture(GL_TEXTURE_2D, textures_earth[1]);
