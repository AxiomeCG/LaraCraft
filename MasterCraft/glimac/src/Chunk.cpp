    ...
    for (int z = 0; z < LENGTH; ++z) {
      for (int height = 0; height < (int)localHeightMap.at(x).at(z); height++) {
        ...
        for (int v = 0; v < cube.getVertexCount(); ++v) {
          ...
          double vertexX = currentCubeVertex.position.x /
                           2.; // The cube is defined from -1 to 1 so it's
                               // size 2. Divide by two to get a 1x1x1 size cube
          double vertexY = currentCubeVertex.position.y / 2.;
          double vertexZ = currentCubeVertex.position.z / 2.;

          shapeVertex.position =
              vec3(vertexX + (float)x + positionTranslation.x,
          ...
          shapeVertex.texCoords =
              vec2(localOffsetTextureMap.at(x).at(z) +
                       currentCubeVertex.texCoords.x /
...
void Chunk::searchForNeighborhood() {
  ...
  for (int x = 0; x < WIDTH; ++x) {
    for (int z = 0; z < LENGTH; ++z) {
      for (int height = 0; height < (int)localHeightMap.at(x).at(z); height++) {
        if (isSurroundedByNeighbors(x, z, height))
          clone[x][z][height] = false;
      }
    }
  }
...
GLsizei Chunk::getVertexCount() const { return m_Vertices.size(); }

std::vector<glimac::ShapeVertex> Chunk::getDataVector() const {
  return m_Vertices;
}

const ShapeVertex *Chunk::getDataPointer() const { return &m_Vertices[0]; }

bool Chunk::isSurroundedByNeighbors(int x, int z, int height) {
  bool isAboveFilled =
      height + 1 < HEIGHT && isFilled3DVector[x][z][height + 1];
  ...
  return isAboveFilled && isBelowFilled && isLeftFilled && isRightFilled &&
