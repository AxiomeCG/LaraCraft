...
bool MovementChecker::willMoveToAnotherCube(glm::vec3 currentPosition,
  ...
  if ((futurePosFlooredX == currentPosFlooredX) &&
      (futurePosFlooredZ == currentPosFlooredZ)) {
    return false;
  }
  return true;
}

bool MovementChecker::canMoveToTheOtherCube(glm::vec3 currentPosition,
                                            glm::vec3 futurePosition) {
  if (isWithinBoundaries(futurePosition)) {
    ...
    auto currentPosFlooredX = floor(currentPosition.x);
    auto currentPosFlooredZ = floor(currentPosition.z);
    auto heightDifference =
        m_heightData[futurePosFlooredX][futurePosFlooredZ] -
...
glm::vec3 MovementChecker::leveledFuturePosition(glm::vec3 currentPosition,
                                                 glm::vec3 futurePosition,
                                                 float heightOfEntity) {
  auto futurePosFlooredX = floor(futurePosition.x);
  auto futurePosFlooredZ = floor(futurePosition.z);
  if (willMoveToAnotherCube(currentPosition, futurePosition)) {
    if (canMoveToTheOtherCube(currentPosition, futurePosition)) {
      auto futureY =
          floor((float)m_heightData[futurePosFlooredX][futurePosFlooredZ]);
      return glm::vec3(futurePosition.x, futureY + heightOfEntity,
                       futurePosition.z);
    }
