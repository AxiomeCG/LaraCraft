...
GLFWWindowManager::GLFWWindowManager(int width, int height, const char *appName,
                                     windowModes::WindowMode windowMode)
...
GLFWwindow *GLFWWindowManager::getWindow() { return window; }

int GLFWWindowManager::windowShouldClose() {
  return glfwWindowShouldClose(window);
}

void GLFWWindowManager::swapBuffers() { glfwSwapBuffers(window); }
...
int GLFWWindowManager::getKey(int key) { return glfwGetKey(window, key); }

void GLFWWindowManager::getCursorPosition(double *pX, double *pY) {
  glfwGetCursorPos(window, pX, pY);
}

void GLFWWindowManager::handleEventsForFPSConstrainedView(
    glimac::ConstrainedCamera &camera, float angle) {
  glfwPollEvents();
