...
mat4 Pnj::getModelMatrix() const {
  ...
  mat4 R = rotate(mat4(), (float)atan2(direction.z, direction.x),
                  vec3(0.f, 1.f, 0.f));
...
void Pnj::updatePosition() {
  ...
  // bool willMove = movementChecker.willMoveToAnotherCube(position,
  // tmpPosition);
  ...
  position = movementChecker.leveledFuturePosition(position, tmpPosition, 1.);
}
