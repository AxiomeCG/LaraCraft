...
OffsetTextureMap::OffsetTextureMap(const ColorMap &colorMap)
    : width(colorMap.getWidth()), height(colorMap.getHeight()),
      offsetData(colorMap.getColorData().size()) {
  auto data = colorMap.getColorData();
  for (int i = 0; i < data.size(); i++) {
    std::vector<float> textureColumn(data[i].size(),
                                     TextureOffsetEnum::dirtTextureOffset);
...
unsigned int OffsetTextureMap::getWidth() const { return width; }

unsigned int OffsetTextureMap::getHeight() const { return height; }
