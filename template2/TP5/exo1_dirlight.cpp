...
int main(int argc, char** argv) {
    ...
    GLuint locateReflexionDiffuse = glGetUniformLocation(program.getGLId(), "uKd");
    GLuint locateReflexionGlossy = glGetUniformLocation(program.getGLId(), "uKs");
    GLuint locateShinny = glGetUniformLocation(program.getGLId(), "uShininess");
    GLuint locateLightDirect = glGetUniformLocation(program.getGLId(), "uLightDir_vs");
    GLuint locateLightIntensity = glGetUniformLocation(program.getGLId(), "uLightIntensity");

    std::unique_ptr<Image> earth_image = loadImage("/Users/mhudelot/Documents/Synthese/GLImac-Template/TP5/asset/textures/EarthMap.jpg");
    if(earth_image == NULL) {
        std::cerr << "Failed to load Image" << std::endl;
        return EXIT_FAILURE;
    }

    std::unique_ptr<Image> moon_image = loadImage("/Users/mhudelot/Documents/Synthese/GLImac-Template/TP5/asset/textures/MoonMap.jpg");
    if(moon_image == NULL) {
        std::cerr << "Failed to load Image" << std::endl;
        return EXIT_FAILURE;
    }
