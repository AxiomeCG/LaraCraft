...
int main(int argc, char **argv) {
    ...
    while (!done) {
        // Event loop:
        SDL_Event e;
        float camera_forward = 0.f;
        float camera_left = 0.f;
        float camera_up = 0.f;
            ...
            switch (e.type){
                ...
                case SDL_MOUSEMOTION:
                    if (isRightClickPressed){
                        camera_left += (float) e.motion.xrel;
                        camera_up += (float) e.motion.yrel;
                    }
                    break;
        ...
        camera.moveFront(camera_forward/10);
        camera.rotateLeft(camera_left/2);
        camera.rotateUp(camera_up/2);
